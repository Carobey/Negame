    ...
    try {
        ...
        return db_->executeTransaction([&](pqxx::work& txn) {
            ...
            for (const auto& field : updateMask) {
                ...
                // TODO: добавить маппинг полей properties на значения
            ...
            auto result = txn.exec_params(
                query,
                id
                // TODO: добавить параметры из paramValues
            );
...
bool CelestialObjectRepositoryImpl::validateObject(const CelestialObject& object) const {
    ...
    switch (object.type()) {
        case CelestialObjectType::PLANET:
            ...
            break;
        // Добавить проверки для других типов
        default:
            break;
    }
...
CelestialObject CelestialObjectRepositoryImpl::rowToObject(const pqxx::row& row, core::Logger& logger) {
    try {
        CelestialObject obj;
        ...
        if (!row["properties"].is_null()) {
            auto props_json = boost::json::parse(row["properties"].as<std::string>());
            auto* props = obj.mutable_properties();
            
            if (props_json.is_object()) {
                const auto& obj = props_json.as_object();
                for (const auto& [key, value] : obj) {
                    if (value.is_string()) {
                        (*props)[std::string(key)] = std::string(value.as_string());
                    } else {
                        (*props)[std::string(key)] = boost::json::serialize(value);
                    }
                }
            }
        }
...
CelestialObjectProperty CelestialObjectRepositoryImpl::rowToProperties(const pqxx::row& row, core::Logger& logger) {
    try {
